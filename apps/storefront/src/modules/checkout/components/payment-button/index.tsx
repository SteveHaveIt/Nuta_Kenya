...
// M-PESA Link - Fallback option using Paynector M-PESA link
const MpesaLinkFallback = ({
  ...
  const handleMpesaLink = async () => {
    ...
    try {
      ...
      // Create Paynector M-PESA payment link
      const paynectorLink = `https://paynecta.co.ke/pay/nuta-kenya?amount=${amount}&phone=${formattedPhone}&ref=${orderId}&currency=KES`
      
      // Open the Paynector payment page
      window.open(paynectorLink, '_blank')
      setLinkOpened(true)
    } catch (err: any) {
      setErrorMessage(err.message || "Failed to open payment page")
    } finally {
      setSubmitting(false)
    }
  }
    ...
    return (
      <div className="bg-green-50 border border-green-200 rounded-lg p-4">
        <p className="text-green-700 text-sm font-medium">
          ✓ Paynector payment page opened!
        </p>
        <p className="text-green-600 text-xs mt-1">
          Complete your payment on the Paynector page. Your order will be processed once payment is confirmed.
        </p>
      </div>
    )
  }
    ...
    return (
      <div className="flex flex-col gap-2 border-t border-gray-200 pt-4 mt-4">
        <p className="text-sm text-ui-fg-subtle">
          Alternative: Pay via Paynector M-PESA
        </p>
        <input
          type="tel"
  ...
  return (
    <div className="border-t border-gray-200 pt-4 mt-4">
      <Button
        ...
        size="large"
      >
        Pay via Paynector M-PESA (Alternative)
      </Button>
    </div>
  )
}
